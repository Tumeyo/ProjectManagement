...
const TolborTolow = () => {
  ...
  useEffect(() => {
    const fetchPaymentStates = async () => {
      try {
        const response = await fetch("/api/paymentstate");
        if (!response.ok) {
          throw new Error("Failed to fetch payment states");
        }
        const data = await response.json();
        const initializedStates = data.map((item) => ({ ...item, isEditing: false }));
        setPaymentStates(initializedStates);
      } catch (err) {
  ...
  const saveChanges = async (state) => {
    try {
      const response = await fetch("/api/paymentstate", {
        ...
        body: JSON.stringify({
          Tulbur_tuluv_lavlah_id: state.Tulbur_tuluv_lavlah_id,
          Tulbur_tuluv_ner: state.Tusliin_tuluv_ner,
        }),
      });
  ...
  const deletePaymentState = async (id) => {
    try {
      const response = await fetch("/api/paymentstate", {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ Tulbur_tuluv_lavlah_id: id }),
      });
  
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.message || "Failed to delete payment state");
      }
      ...
      setPaymentStates((prevStates) =>
        prevStates.filter((state) => state.Tulbur_tuluv_lavlah_id !== id)
      );
    } catch (error) {
      console.error("Error deleting payment state:", error.message);
    }
  };
